...
const intents = new Discord.Intents([ GUILDS, GUILD_VOICE_STATES ]);
...
const url = 'https://www.youtube.com/watch?v=uQQgZp9Ylbiio';
...
player.on(AudioPlayerStatus.Idle, () => {
	client.user.setUsername('ム. (Idle)');
});
player.on(AudioPlayerStatus.Playing, () => {
	client.user.setUsername('ム.');
});
player.on(AudioPlayerStatus.AutoPaused, () => {
	client.user.setUsername('ム. (Paused)');
});
	...
	if (message.content.startsWith(prefix + ' again')) {
		...
		function voiceRestart(guildid, channelid) {
			...
			message.channel.send({
				embed: new Discord.MessageEmbed().addField(`starting again.`, true),
			});
		}
